...
<h1>Current Tendors</h1>
</div>

@foreach($tendor as $key => $item)
{{-- 
@if (empty($item->tendorend))
    Currently there is no Tendors Found
@endif --}}
...
<p>{{$item->name}} {{$item->lname}}</p>
         ...
         <img class="" height = "80px" width = "130px" src="{{ asset('storage/tendor_sample_image/'.$item->sample_image) }}" alt="">           
...
<div class = "react_title">
<h1>Reacted Tendors</h1>
</div>
  {{-- actio tendor portion --}}
@foreach($tendor_action as $key => $item)
...
<p>{{$item->sub_category}}</p>
      ...
      <p class = "date_time">{{ $item->tendorend }}</p>
      <p>
     <a href = "{{route('tendor_action_removed',$item->tendoractionId)}}" > <i class="fas fa-times"></i></a>
      </p>
      </div>
...
@endif
         ...
         <img class="mt-3" height = "80px" width = "130px" src="{{ asset('storage/tendor_sample_image/'.$item->sample_image) }}" alt="">           
